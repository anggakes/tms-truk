          ...
          <div class="box sms box-default color-palette-box">
            ...
            <div id="wrapper-console" class="clearfix">
                <div id="wrapper-search">
                             <form action="<?php echo base_url()."index.php/trucking_order" ?>" method="get">
...
else
{
	...
	callTransportOrder(trip_url,delivery_date_url);
	callManifest(trip_url,delivery_date_url);
...
function callManifest(trip,delivery_date){
	
		var delivery_date = delivery_date[2]+'-'+delivery_date[1]+'-'+delivery_date[0];
		$.ajax({
		  url: 'http://localhost/tms/index.php/json/jsonManifestRoutePlanning?delivery_date='+delivery_date+'&&trip='+trip,
...
function callTransportOrder(trip,delivery_date){
		
		var delivery_date = delivery_date[2]+'-'+delivery_date[1]+'-'+delivery_date[0];
		$.ajax({
		  url: 'http://localhost/tms/index.php/json/jsonTransportOrderRoutePlanning?delivery_date='+delivery_date+'&&trip='+trip,
		  success: function(data,status)
		  {
...
function createTableTransportOrder(data)
{
  for(var i=0; i<data.length;i++)
  {
		...
		$(".table_transport_order tbody").append("<tr spk_number='"+data[i]['spk_number']+"' class='drag draggable "+data[i]['spk_number']+" '>"+
													"<td><div class='delete_transport_order' confirmed_manifest='no' manifest='"+data[i]['manifest']+"' spk_number='"+data[i]['spk_number']+"' >Delete</div></td>"+
...
function createTableManifest(data)
{
  for(var i=0; i<data.length;i++)
  {
				...
				$("#table_drop_received tbody").append("<tr class='tr droppable' manifest='"+data[i]['manifest']+"' >"+
					"<td><div class='delete_manifest' confirmed_manifest='"+data[i]['confirmed_manifest']+"' manifest='"+data[i]['manifest']+"'>Delete</div></td><td colspan='16' manifest='"+data[i]['manifest']+"' class='"+data[i]['manifest']+" vehicle_color '>vehicle ID ====> "+data[i]['vehicle_id']+"vehicle Type ====> "+data[i]['vehicle_type']+"Description ====> "+data[i]['description']+"</td>"+
					"</tr>");
					...
					$(".tr.droppable").droppable({
									...
									drop: function(event, ui) {
										...
										$.ajax({
											type: "POST",
											url: "http://localhost/tms/index.php/route_planning/updateManifest",
...
function callTransportOrderManifest(trip,delivery_date,manifest,status_confirmed){
		...
		$.ajax({
		  url: 'http://localhost/tms/index.php/json/jsonTransportOrderManifestRoutePlanning?delivery_date='+delivery_date+'&&trip='+trip+'&&manifest='+manifest,
		  success: function(data,status)
		  {
...
function createTransportOrderManifest(data,status_confirmed)
{
  for(var i=0; i<data.length;i++)
  {
		$("#table_drop_received tbody tr[manifest='"+data[i]['manifest']+"']").after("<tr spk_number='"+data[i]['spk_number']+"' class='drag draggable "+data[i]['spk_number']+" '>"+
													"<td><div class='delete_transport_order' confirmed_manifest='"+status_confirmed+"' manifest='"+data[i]['manifest']+"' spk_number='"+data[i]['spk_number']+"' >Delete</div></td>"+
													"<td>"+data[i]['spk_number']+"</td>"+
													"<td>"+data[i]['do_number']+"</td>"+
													"<td>"+data[i]['reference']+"</td>"+
													"<td>"+data[i]['delivery_date']+"</td>"+
...
$( "#guest-list" ).on( "click", ".delete_manifest", function() {
	...
	if(confirmed_manifest!='yes')
	{
		if (confirm('Delete Manifest?')) {
			...
			$.ajax({
				type: "POST",
				url: "http://localhost/tms/index.php/route_planning/deleteManifest",
...
$( "#guest-list" ).on( "click", ".delete_transport_order", function() {
	...
	if (confirm('unroute Transport Order?')) {
		...
		$.ajax({
			type: "POST",
			url: "http://localhost/tms/index.php/route_planning/deleteTransportOrder",
...
$("#search_button_vehicle").click(function(){
	...
	$.ajax({
      url: 'http://localhost/tms/index.php/json/jsonMasterUnitRoutePlanning?search='+vehicle_id,
      success: function(data,status)
      {
...
$("#confirmed_manifest").click(function(){
		...
		$.ajax({
		  url: 'http://localhost/tms/index.php/json/jsonManifest?manifest='+manifest,
		  success: function(data,status)
		  {
			...
			var confirmed_rate = data[0]['confirmed_rate'];
				...
				{	
					...
					{
						$("#form_confirmed_manifest").trigger('reset');				
						alert("Please confirmed Additional Cost before!"); 
					}
		...
		$.ajax({
		  ...
		  success: function(data,status)
		  {
			...
			{	
				...
				{
					$.ajax({
					  url: 'http://localhost/tms/index.php/json/jsonAdditionalCost?id_manifest='+id_manifest,
					  success: function(data,status)
					  {
...
function createTableDetail(data)
			{
			  for(var i=0; i<data.length;i++)
			  {
					...
					$(".po tbody").append("<tr><td><input type='checkbox' class='edit_case'/></td>"+
											 ...
											 "</select></td>"+
											 "<td><input type='text' class='amount' name='amount[]' value='"+data[i]['amount']+"'/></td>"+
											 "<td><input type='text' class='description' name='description[]' value='"+data[i]['description']+"'/></td>"+
											 "</tr>");
...
$("#submit_add_manifest").click(function(){
	...
	$( "tr.active" ).each(function( index ) {
		...
		var xhr = "<?php echo $this->security->get_csrf_hash();?>";
			$.ajax({
			type: "POST",
			url: "http://localhost/tms/index.php/route_planning/saveManifest",
			...
			success: function(data){
					...
					if(status=='sukses')
					{
					var manifest = $(data).filter('div#manifest').text();
					$("#table_drop_received tbody").append("<tr class='tr droppable' manifest='"+manifest+"' >"+
					"<td><div class='delete_manifest' confirmed_manifest='no' manifest='"+manifest+"'>Delete</div></td><td manifest='"+manifest+"' colspan='16' class='"+manifest+" vehicle_color '>vehicle ID ====> "+vehicle_id+"vehicle Type ====> "+vehicle_type+"Description ====> "+description+"</td>"+
					"</tr>");
...
<script>
        ...
        $( "#search_driver" ).autocomplete({
         source: "http://localhost/tms/index.php/json/jsonDriver",  
         minLength:0,
		 select: function( event , ui ) {
        ...
        $( "#search_trucking_order" ).autocomplete({
         source: "http://localhost/tms/index.php/json/jsonTruckingOrder",  
         minLength:0,
		 select: function( event , ui ) {
...
function addrow(){
    ...
    var data="<tr><td><input type='checkbox' class='edit_case'/></td>"+
			 ...
			 "<option value='Multidrop Outer City'>Multidrop Outer City</option>"+
			 "</select></td>"+
			 "<td><input type='text' class='amount' name='amount[]' value=''/></td>"+
