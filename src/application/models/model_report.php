...
class Model_report extends CI_Model{
	...
	public function dataDailySummary($sampai,$dari,$search_term="",$user_role,$code_role,$date,$regional,$motorist_type){
		...
		$this->db->select(" * ,(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND s.day_visit = '".$day."') as target_call,
	...
	public function journeyMotorist($sampai,$dari,$search_term="",$user_role,$code_role,$regional,$motorist_type){
		...
		$this->db->select(" * ,(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND frequency = 'f2' AND s.distributor_code = motorist.distributor_code AND s.day_visit = 'monday') as f2_monday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F1' AND s.day_visit = 'monday') as f1_monday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'W' AND s.day_visit = 'monday') as w_monday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F1' AND s.day_visit = 'tuesday') as f1_tuesday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F2' AND s.day_visit = 'tuesday') as f2_tuesday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'W' AND s.day_visit = 'tuesday') as w_tuesday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F2' AND s.day_visit = 'wednesday') as f2_wednesday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F1' AND s.day_visit = 'wednesday') as f1_wednesday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'W' AND s.day_visit = 'wednesday') as w_wednesday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F2' AND s.day_visit = 'thursday') as f2_thursday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F1' AND s.day_visit = 'thursday') as f1_thursday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'W' AND s.day_visit = 'thursday') as w_thursday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F2' AND s.day_visit = 'friday') as f2_friday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F1' AND s.day_visit = 'friday') as f1_friday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'W' AND s.day_visit = 'friday') as w_friday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F2' AND s.day_visit = 'saturday') as f2_saturday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code  AND frequency = 'W' AND s.day_visit = 'saturday') as w_saturday,
		(SELECT COUNT(customer_code) FROM store as s WHERE s.motorist_code = motorist.motorist_code AND s.distributor_code = motorist.distributor_code AND frequency = 'F1' AND s.day_visit = 'saturday') as f1_saturday
	...
	{
		$data = $this->db->query("SELECT * FROM email_receiver ".$where);
		return $data;
	}
	
	
	public function getLogReport($where=" ")
	{
		$data = $this->db->query("SELECT * FROM log_report ".$where);
		return $data;
	}
