...
class Model_area extends CI_Model{
	...
	public function countArea($search_term=""){
		$this->db->or_like('area_id', $search_term);
		return $this->db->get('master_area')->num_rows();
	}
	
	public function dataArea($sampai,$dari,$search_term=""){
		$this->db->or_like('area_id', $search_term);
		return $query = $this->db->get('master_area',$sampai,$dari)->result();
