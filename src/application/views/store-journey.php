 ...
 <section class="content-header">
          <h1>
            Journey Plant
            <small>Store Journey</small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="<?php echo BASE_URL(); ?>"><i class="fa fa-dashboard"></i>Home</a></li>
            <li><a href="<?php echo BASE_URL()."journeyPlanMotorist"; ?>"><i class="fa fa-dashboard"></i>Store Journey</a></li>
          </ol>
        </section>
   ...
   <section class="content">
          <div class="box sms box-default color-palette-box">
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-table"></i>Table Store Journey Plan</h3>
            </div>
            <div class="box-body">
			<?php
			...
			$Frequency = isset($_GET['frequently']) ? $_GET['frequently'] : '';
            ...
            <h3>Motorist Code: <?php echo $data_motorist[0]['motorist_code']; ?></h3>
         	<h3>Distributor Code: <?php echo $data_motorist[0]['motorist_code']; ?></h3>
            <h3>Day Visit: <?php echo $day_visit; ?></h3>
            <h3>Frequency: <?php echo $Frequency; ?></h3>
              <div id="wrapper-button" class="clearfix"  style="margin-top:20px; width:100%; float:none;">
                	<a href="<?php echo base_url()."index.php/motorist/exportStoreJourney?motorist_code=".$motorist_code."&&distributor_code=".$distributor_code."&&day_visit=".$day_visit."&&frequently=".$Frequency; ?>" class="button orange-button">
                    	Export Store Journey
                    </a>
                    
                </div>
            <form action="<?php echo base_url()."index.php/store/updateStoreKa" ?>" method="POST">
            <input style="display:inline-block; padding:10px; color:#FFF; width:250px; text-align:center; margin-bottom:10px;" type="submit" class="button green-button"  value="Update Frequency & Day Visit"/>
           	...
           	<table id="myTable03" class="fancyTable table  table-bordered table-hover">
                    <thead>
                      <tr>
                        ...
                        <th>Update Day Visit</th>
                        <th>Update Customer Status</th>
                      </tr>
                    </thead>
                    
                     <?php 
					 $no = 1;
					 foreach($data_store as $data_store1) {	
                        ...
                        <td><?php echo $no; ?></td>
                        <td><?php echo $data_store1->customer_name;  ?></td>
                        <td><?php echo $data_store1->customer_code;  ?></td>
                        <td><?php echo $data_store1->address;  ?></td>
                        <td><?php echo $data_store1->districts;  ?></td>
                        <td><?php echo $data_store1->customer_contact;  ?></td>
                        <td><?php 
						$select_mtd_sales = $this->db->query("SELECT SUM(total_order) as total_order FROM orders WHERE customer_code = '".$data_store1->customer_code."' AND date BETWEEN '".$tanggal_bulan_awal."' AND '".$tanggal_bulan_akhir."' ")->result_array();
						if($select_mtd_sales[0]['total_order']=='')
						{convert_price(0);}
						else
						{echo convert_price($select_mtd_sales[0]['total_order']);}
						
						?></td>
                        <td><?php echo $data_store1->kelurahan;  ?></td>
<td>
                        <input type="hidden" name="id_store[]" value="<?php echo $data_store1->id_store; ?>" />
                        <select name="status_toko[]">
                        <option <?php if($data_store1->frequency=="w" || $data_store1->frequency=="W"){echo'selected=selected';} ?> value="W">W</option>
                        <option <?php if($data_store1->frequency=="f1" || $data_store1->frequency=="F1"){echo'selected=selected';} ?>  value="F1">F1</option>
                        <option <?php if($data_store1->frequency=="f2" || $data_store1->frequency=="F2"){echo'selected=selected';} ?>  value="F2">F2</option>
                        </select></td>
                        <td>
                        <select name="day_visit[]">
                        <option <?php if($data_store1->day_visit=="monday"){echo'selected=selected';} ?> value="monday">Monday</option>
                        <option <?php if($data_store1->day_visit=="tuesday"){echo'selected=selected';} ?>  value="tuesday">Tuesday</option>
                        <option <?php if($data_store1->day_visit=="wednesday"){echo'selected=selected';} ?>  value="wednesday">Wednesday</option>
                        <option <?php if($data_store1->day_visit=="thursday"){echo'selected=selected';} ?>  value="thursday">Thursday</option>
                        <option <?php if($data_store1->day_visit=="friday"){echo'selected=selected';} ?>  value="friday">Friday</option>
                        <option <?php if($data_store1->day_visit=="saturday"){echo'selected=selected';} ?>  value="saturday">Saturday</option>
                        </select></td>
                        
                        
                         <td>
                        <select name="customer_status[]">
                        <option <?php if($data_store1->customer_status=="active"){echo'selected=selected';} ?> value="active">Active</option>
                        <option <?php if($data_store1->customer_status=="no_active"){echo'selected=selected';} ?>  value="no_active">No Active</option>
                        </select></td>
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCoxhCpa75Oe6014hzt4R_2O-YZIW9gunY&callback=setup" async defered></script>
<script type="text/javascript">

 (function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.setup = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
module.exports = function() {
 ...
 <?php 
  $no_2 = 1;
  foreach($data_store as $data_store2) {	
  ?>
  var loan = "<?php echo $data_store2->customer_name; ?>"
  var latLng = new google.maps.LatLng(<?php echo "$data_store2->latitude"; ?>, <?php echo "$data_store2->longitude"; ?>);
  var marker = new MarkerWithLabel({
    ...
    labelContent: "<?php echo $no_2; ?>",
    labelAnchor: new google.maps.Point(4, 40),
    labelClass: "labels", // the CSS class for the label
    labelInBackground: false
  });

  var content =   "<?php echo $no_2; ?>. Toko " + loan
