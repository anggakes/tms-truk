            ...
            <div id="wrapper-console" class="clearfix">
                <div id="wrapper-search">
                             ...
                             <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash();?>" />
                                ...
                                <?php if($search!="")
								{?>
                                <a href="<?php echo base_url()."index.php/trucking_order" ?>" id="clear-search">Clear Search</a>
                                <?php } ?>
                             </form>
                </div>
                <div id="wrapper-button" class="clearfix">
					<?php if($data_role[0]['export_trucking_order']=='yes'){ ?>
                	<a href="<?php echo base_url()."index.php/trucking_order/exportTrucking Order?search=".$search; ?>" class="button orange-button">
                    	Export Trucking Order
                    </a>
					<?php } ?>
                    <?php if($data_role[0]['add_trucking_order']=='yes'){ ?>
                    <a id="add-data" class="button green-button">
                    	Add Trucking Order
                    </a>
					<?php } ?>
					<?php if($data_role[0]['delete_trucking_order']=='yes'){ ?>
                    <a id="delete-data" class="button btn-danger deletelogg">
                    	Delete Trucking Order
                    </a>
					<?php } ?>
					<!--<?php if($data_role[0]['import_trucking_order']=='yes'){ ?>
  ...
  $(".detail_data_manifest").click(function(){
	...
	else
	{
		$('[data-remodal-id = modal_detail]').remodal().open();
			...
			$.ajax({
				  url: '<?php echo BASE_URL(); ?>/index.php/json/jsonDetailManifest?id_manifest='+id_manifest,
