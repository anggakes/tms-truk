...
class Area extends CI_Controller {
	...
	public function deleteAreaAll()
	{
			...
			if($data_role[0]['delete_area']=='yes')
			{
				
			
				// Get Variable Post
				$id_area_delete = $_POST['id_area_delete_all'];
				
		
					//Update Product
					$res = $this->db->query("DELETE FROM master_area WHERE id_area in (".$id_area_delete.") ");
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data Area has been successfully deleted!");
						redirect('area');
						
						}
					else
					{
						$this->session->set_flashdata('message_failed',"Data Area failed to delete!");
						redirect('area');
					}
	...
	public function editArea()
	{
			...
			if($data_role[0]['edit_area']=='yes')
			{
				
			
				// Get Variable Post
				$area_id = $_POST['edit_area_id'];
				$area_description = $_POST['edit_area_description'];
				$area_type = $_POST['edit_area_type'];
				$id_area = $_POST['id_area_update'];
				$data_update = array(
				...
				$check = $this->model_area->getArea("WHERE area_id = '".$area_id."' AND id_area != '".$id_area."' ");
				...
				else
				{
					...
					//Update
					$res = $this->model_area->UpdateData("master_area",$data_update,$where);
					
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data Area has been successfully updated!");
					...
					{
						$this->session->set_flashdata('message_failed',"Data Area failed to update!");
						redirect('area');
					}
				}
	...
	public function deleteArea()
	{
			...
			if($data_role[0]['delete_area']=='yes')
			{
				...
				// Get Variable Post
				$id_area_delete = $_POST['id_area_delete'];
				$where = array("id_area" => $id_area_delete);
				
		
					//Update Product
					$res = $this->model_area->DeleteData("master_area",$where);
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data Area has been successfully deleted!");
						redirect('area');
	...
	public function exportArea()
	{
		...
		if($data_role[0]['export_area']=='yes')
		{
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, 1, "Master Area");
        ...
        foreach($query->result_array() as $data)
        {
			...
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $row, $data['area_id']);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, $data['area_description']);
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $data['area_type']);
	...
	public function importArea()
	{
			...
			if($data_role[0]['import_area']=='yes')
			{
			...
			for ($row = 2; $row <= $highestRow; $row++){  				//  Read a row of data into an array 				
					...
					$check = $this->model_area->getArea("WHERE area_id  = '".$rowData[0][0]."' ");
					if($check->num_rows() >=1 )
					{
					...
					else
					{
						...
						//Insert Data
						$res = $this->model_area->insertData("master_area",$data_insert);
	...
	public function addArea()
	{
			...
			if($data_role[0]['add_area']=='yes')
			{
				...
				//Check Product Apakah sudah ada dengan kode produk dan type motorist yang sama
				$check = $this->model_area->getArea("WHERE area_id = '".$area_id."' ");
				if($check->num_rows() >=1 )
				{
					$this->session->set_flashdata('message_failed',"Sorry data already exist!");
					redirect('area');
				...
				else
				{
					...
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data Area has been successfully saved!");
						redirect('area');
						}
	...
	public function index()
	{
			...
			$jumlah= $this->model_area->countArea($search);
			...
			$data = $this->model_area->dataArea($config['per_page'],$dari,$search);
			$this->pagination->initialize($config);
			
			$data_category = $this->db->query("SELECT * FROM master_data_category WHERE category = 'Area Type' ")->result();
		
			$comp = array(
				'title' => ' Area',
				'content' => $this->load->view('area',array('data_category'=>$data_category,'data_area'=>$data,'data_role'=>$data_role,'search'=>$search),true),
				'sidebar' => $this->html_sidebar()
			);
			$this->load->view("common/common",$comp);
			}
