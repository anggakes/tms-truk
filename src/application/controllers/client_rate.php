...
class Client_rate extends CI_Controller {

	function __construct()  {
 		parent::__construct();
			$this->load->library(array('PHPExcel','PHPExcel/IOFactory'));
			$this->load->model('model_account_master');
			$this->load->model('model_client_rate');
			$this->load->library('tank_auth');
			
	}
	
	
	public function deleteClientRateAll()
	{
			...
			if($data_role[0]['delete_client_rate']=='yes')
			{
				
			
				// Get Variable Post
				$id_client_rate_delete = $_POST['id_client_rate_delete_all'];
				
		
					//Update Product
					$res = $this->db->query("DELETE FROM client_rate WHERE id_client_rate in (".$id_client_rate_delete.") ");
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data client_rate has been successfully deleted!");
						redirect('client_rate');
						
						}
					else
					{
						$this->session->set_flashdata('message_failed',"Data ClientRate failed to delete!");
						redirect('client_rate');
					}
	...
	public function editClientRate()
	{
			...
			if($data_role[0]['edit_direct_cost']=='yes')
			{
				...
				$period_rate = $_POST['edit_period_rate'];
				...
				$uov = $_POST['edit_uov'];
				$id_client_rate = $_POST['id_client_rate_update'];
				...
				$where = array("id_client_rate"=>$id_client_rate);
				
				//Check Product Apakah sudah ada dengan kode produk dan type motorist yang sama
				$check = $this->model_client_rate->getClientRate("WHERE client_id = '".$client_id."' AND origin = '".$origin."' AND destination = '".$destination."' AND id_client_rate != '".$id_client_rate."' ");
				...
				else
				{
					...
					//Update
					$res = $this->model_client_rate->UpdateData("client_rate",$data_update,$where);
	...
	public function deleteClientRate()
	{
			...
			if($data_role[0]['delete_client_rate']=='yes')
			{
				...
				// Get Variable Post
				$id_client_rate_delete = $_POST['id_client_rate_delete'];
				$where = array("id_client_rate" => $id_client_rate_delete);
				
		
					//Update Product
					$res = $this->model_client_rate->DeleteData("client_rate",$where);
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data client_rate has been successfully deleted!");
						redirect('client_rate');
	...
	public function exportClientRate()
	{
	   ...
	   // Sending headers to force the user to download the file
        ...
        header('Content-Disposition: attachment;filename="Master-client-rate-'.$date.'.xls"');
	...
	public function importClientRate()
	{
			...
			if($data_role[0]['import_client_rate']=='yes')
			{
			...
			for ($row = 2; $row <= $highestRow; $row++){  				//  Read a row of data into an array 				
					...
					$check = $this->model_client_rate->getClientRate("WHERE client_ratecode  = '".$rowData[0][0]."' ");
					if($check->num_rows() >=1 )
					{
						
						$data_update = array(
							"client_ratename" => $rowData[0][1],
							"password" => md5($rowData[0][2])
							
						);
						$where = array(
							"client_ratecode" => $rowData[0][0]
						);
					...
					else
					{
						...
						//Insert Data
						$res = $this->model_client_rate->insertData("client_rate",$data_insert);
	...
	public function addClientRate()
	{
			...
			if($data_role[0]['add_client_rate']=='yes')
			{
				...
				//Check Product Apakah sudah ada dengan kode produk dan type motorist yang sama
				$check = $this->model_client_rate->getClientRate("WHERE client_id = '".$client_id."' AND origin = '".$origin."' AND destination = '".$destination."'  ");
				if($check->num_rows() >=1 )
				{
					$this->session->set_flashdata('message_failed',"Sorry data already exist!");
					redirect('client_rate');
				...
				else
				{
					...
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data client_rate has been successfully saved!");
						redirect('client_rate');
						}
	...
	public function index()
	{
			...
			if($data_role[0]['see_client_rate']=='yes')
			{	
			...
			$jumlah= $this->model_client_rate->countClientRate($search);
			
			if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
			$config['base_url'] = base_url().'index.php/client_rate/client_rate/';
			...
			$data = $this->model_client_rate->dataClientRate($config['per_page'],$dari,$search);
			
			$data_category = $this->db->query("SELECT * FROM master_data_category WHERE category = 'Vehicle Status Client Rate' ")->result();
			
		
			$comp = array(
				'title' => ' ClientRate',
				'content' => $this->load->view('client_rate',array('data_category'=>$data_category,'data_client_rate'=>$data,'data_role'=>$data_role,'search'=>$search),true),
				'sidebar' => $this->html_sidebar()
			);
			$this->load->view("common/common",$comp);
			}
