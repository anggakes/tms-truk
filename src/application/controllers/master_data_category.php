...
class Master_data_category extends CI_Controller {

	function __construct()  {
 		parent::__construct();
			$this->load->library(array('PHPExcel','PHPExcel/IOFactory'));
			$this->load->model('model_account_master');
			$this->load->model('model_master_data_category');
			$this->load->library('tank_auth');
			
	}
	
	
	public function deleteMasterDataCategoryAll()
	{
			...
			if($data_role[0]['delete_master_category']=='yes')
			{
				...
				$id_master_data_category_delete = $_POST['id_master_data_category_delete_all'];
					...
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data Master Data Category has been successfully deleted!");
						redirect('master_data_category');
						
						}
					else
					{
						$this->session->set_flashdata('message_failed',"Data Master Data Category failed to delete!");
						redirect('master_data_category');
					}
	...
	public function editMasterDataCategory()
	{
			...
			if($data_role[0]['edit_master_category']=='yes')
			{
				...
				$check = $this->model_master_data_category->getMasterDataCategory("WHERE category = '".$category."' AND description = '".$description."' AND id_master_data_category != '".$id_master_data_category."' ");
				...
				else
				{
					...
					//Update
					$res = $this->model_master_data_category->UpdateData("master_data_category",$data_update,$where);
					
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data Master Data Category has been successfully updated!");
					...
					{
						$this->session->set_flashdata('message_failed',"Data Master Data Category failed to update!");
						redirect('master_data_category');
					}
				}
	...
	public function deleteMasterDataCategory()
	{
			...
			if($data_role[0]['delete_master_category']=='yes')
			{
				...
				$where = array("id_master_data_category" => $id_master_data_category_delete);
					...
					//Update Product
					$res = $this->model_master_data_category->DeleteData("master_data_category",$where);
					if($res>=1)
					{
						$this->session->set_flashdata('message_success',"Data Master Data Category has been successfully deleted!");
						redirect('master_data_category');
	...
	public function exportMasterDataCategory()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, 1, "Master MasterDataCategory");
	...
	public function importMasterDataCategory()
	{
			...
			for ($row = 2; $row <= $highestRow; $row++){  				//  Read a row of data into an array 				
					...
					$check = $this->model_master_data_category->getMasterDataCategory("WHERE area_id  = '".$rowData[0][0]."' ");
					if($check->num_rows() >=1 )
					{
						...
						$res = $this->model_master_data_category->UpdateData("master_master_data_category",$data_update,$where);
					...
					else
					{
						...
						//Insert Data
						$res = $this->model_master_data_category->insertData("master_master_data_category",$data_insert);
	...
	public function addMasterDataCategory()
	{
			...
			if($data_role[0]['add_master_category']=='yes')
			{
				...
				//Check Product Apakah sudah ada dengan kode produk dan type motorist yang sama
				$check = $this->model_master_data_category->getMasterDataCategory("WHERE category = '".$category."' AND description = '".$description."' ");
				if($check->num_rows() >=1 )
				{
					$this->session->set_flashdata('message_failed',"Sorry data already exist!");
					redirect('master_data_category');
				...
				else
				{
					...
					$res = $this->model_master_data_category->insertData("master_data_category",$data_insert);
					if($res>=1)
					{
	...
	public function index()
	{
			...
			$jumlah= $this->model_master_data_category->countMasterDataCategory($search);
			
			if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
			$config['base_url'] = base_url().'index.php/master_data_category/master_data_category/';
			...
			$data = $this->model_master_data_category->dataMasterDataCategory($config['per_page'],$dari,$search);
			$this->pagination->initialize($config);
		
			$comp = array(
				'title' => ' MasterDataCategory',
				'content' => $this->load->view('master_data_category',array('data_master_data_category'=>$data,'data_role'=>$data_role,'search'=>$search),true),
				'sidebar' => $this->html_sidebar()
			);
			$this->load->view("common/common",$comp);
			}
