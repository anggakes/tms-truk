...
class Motorist extends CI_Controller {
	...
	public function dailySalesMotoristAdo2()
	{
			...
			$config['base_url'] = base_url().'index.php/motorist2/dailySalesMotoristAdo/';
			$config['total_rows'] = $jumlah;
			$config['first_link'] = 'First';
			$config['last_link'] = 'End';
			$config['next_link'] = 'Next';
	...
	public function detailCallStrike($id_visit)
	{
			...
			$comp = array(
				...
				'content' => $this->load->view('detail-callstrike',array('data_account'=>$data_account,'data_visit'=>$data_visit,'data_product_orders'=>$data_product_orders),true),
				'sidebar' => $this->html_sidebar()
	...
	public function exportDailySalesMotoristAdo()
	{
	...
	public function exportDailyCallStrike()
	{
        ...
        foreach($query->result_array() as $data)
        {
			$time_in = explode(" ",$data['jam_checkin']);
			$time_in = $time_in[1];
			...
			$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(11, $row, $data['total_order']);
			
			$time_total = $time_total+$time_diff;
			$total_jualan = $total_jualan + $data['total_order'];
		...
		$objPHPExcel->getActiveSheet()->getStyle('B'.$row_total.':L'.$row_total)->applyFromArray(
			...
			$data_channel = $this->db->query("SELECT * FROM visit inner JOIN store on store.customer_code = visit.customer_code WHERE id_motorist = '".$id_motorist."' AND time_stamp_visit LIKE '%".$date_now."%' group by store.channel_code ")->num_rows();
	...
	public function exportDailySummaryMotorist()
	{
        ...
        foreach($query->result_array() as $data)
        {
			...
			$no++;
					...
					$penjualan_harian_footer = $this->db->query("SELECT SUM(total_order) as total_penjualan_harian FROM orders JOIN store on store.customer_code = orders.customer_code WHERE jam_checkin LIKE '%".$date_now."%' ".$regional_store_query." ".$motorist_type_store_query." ".$distributor_store_query." ".$area_store_query." ".$search_query_store." ")->result_array();
					...
					$penjualan_harian_footer = $this->db->query("SELECT SUM(total_order) as total_penjualan_harian FROM orders JOIN store on store.customer_code = orders.customer_code WHERE jam_checkin LIKE '%".$date_now."%' ".$regional_store_query." ".$distributor_store_query." ".$area_store_query." ".$motorist_type_store_query." ".$search_query_store." AND store.distributor_code = '".$data_account[0]['code']."' ")->result_array();
					...
					$penjualan_harian_footer = $this->db->query("SELECT SUM(total_order) as total_penjualan_harian FROM orders JOIN store on store.customer_code = orders.customer_code WHERE jam_checkin LIKE '%".$date_now."%' ".$regional_store_query." ".$search_query_store." ".$motorist_type_store_query." ".$distributor_store_query." ".$area_store_query."  AND store.regional = '".$data_account[0]['code']."' ")->result_array();
